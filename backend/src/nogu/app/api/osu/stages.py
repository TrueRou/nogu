...
async def create_stage(stage: StageBase, team: Team = Security(TeamSrv.require_team, scopes=["member"])):
...
async def patch_stage(stage_update: StageUpdate, stage: Stage = Security(StageSrv.require_stage, scopes=["admin"])):
    with manual_session() as session:
...
async def get_stage_beatmaps(limit=20, offset=0, stage: Stage = Security(StageSrv.require_stage)):
...
async def add_stage_beatmaps(stage_maps: list[StageMapBase], stage: Stage = Security(StageSrv.require_stage, scopes=["admin"])):
    with auto_session() as session:
