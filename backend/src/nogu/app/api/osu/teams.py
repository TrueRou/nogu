...
async def create_team(team: TeamBase, user: User = Depends(UserSrv.require_user)):
    with manual_session() as session:
...
async def patch_team(team_update: TeamBase, team: Team = Security(TeamSrv.require_team, scopes=["admin"])):
    with manual_session() as session:
...
async def get_teams_me(limit: int = 20, offset: int = 0, user: User = Depends(UserSrv.require_user), active_only: bool = False):
...
async def get_scores(limit: int = 20, offset: int = 0, team: Team = Security(TeamSrv.require_team, scopes=["access-sensitive"])):
    with auto_session() as session:
...
async def get_stages(limit: int = 20, offset: int = 0, team: Team = Security(TeamSrv.require_team, scopes=["access-sensitive"])):
    with auto_session() as session:
